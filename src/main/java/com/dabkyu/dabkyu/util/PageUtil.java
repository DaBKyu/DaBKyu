...
public class PageUtil {
		...
		if(totalPage != 1) {
			...
			for(int i=1;i<=pageListCount;i++) {
				...
				if(pageNum != (section-1)*pageListCount +i)
					...
					pageList += " <span style='font-weight: bold'>" + Integer.toString((section-1)*pageListCount+i) + "</span>";
				if(i==pageListCount && totalSection > 1 && totalPage >= i+(section-1)*pageListCount+1)
					pageList += " <a href=/master/reviewList?page=" + Integer.toString(section*pageListCount+1) + "&keyword=" + keyword + ">▶</a>";				
			}			
		}		
		return pageList;
	}
	...
	public String getPageList1(int postNum, int pageListCount, String keyword) {
		...
		int totalPage = (int)Math.ceil((double)postNum);
		int totalSection = (int)Math.ceil(totalPage/(double)pageListCount);
		int section = (int)Math.ceil((double)pageListCount);
		String pageList = "";
